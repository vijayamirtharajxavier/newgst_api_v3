...
{
$sql="SELECT t.statecode, itm.trans_id,itm.trans_date,t.gstin,itm.item_gstpc, round(sum(itm.taxable_amount),2)`txb_amt`,round(sum(itm.nett_amount),2)`net_amt`,round(sum(itm.igst_amount),2)`igst`,round(sum(itm.sgst_amount),2)`sgst`,round(sum(itm.cgst_amount),2)`cgst` FROM `itemtransaction_tbl` itm,transaction_tbl t WHERE itm.delflag=0 AND itm.trans_type='SALE' AND t.id=itm.trans_link_id and t.gstin='' and itm.trans_date>=? and itm.trans_date<=? and itm.company_id=? group by itm.trans_id,itm.item_gstpc ORDER BY itm.trans_id,t.gstin";
$query = $this->db->query($sql, array($fdate,$tdate,$cid));
    return $query->result_array();
}
...
{
$sql_os = "SELECT SUM(CASE WHEN trans_type='SALE' AND (db_account=" . $acct_id . " or cr_account=" . $acct_id . " )  THEN net_amount WHEN trans_type='PYMT' AND (db_account=" . $acct_id . "  or cr_account=" . $acct_id . " ) THEN trans_amount ELSE 0 END)-SUM(CASE WHEN trans_type='PURC' AND (db_account=" . $acct_id . "  or cr_account=" . $acct_id . " )  THEN net_amount WHEN trans_type='RCPT' AND (db_account=" . $acct_id . "  or cr_account=" . $acct_id . " ) THEN trans_amount ELSE 0 END) `outstand` FROM `transaction_tbl` WHERE finyear>=? and company_id=? and delflag=0";
...
public function getLedgerbyId($actid)
{
 if($actid)
 {
 	$sql="SELECT * FROM ledgermaster_tbl where id=?";
